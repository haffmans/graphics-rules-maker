...
void Sims2Settings::autodetect()
{
    ...
    // Sim Shadows: Windows 8 and up, if shadow fix mod is not found
...
bool Sims2Settings::shadowFixModInstalled() const
{
...
#endif
    const auto ts2DisplayName = ts2Settings.value("DisplayName").toString();
#else
    const auto ts2DisplayName = QString("The Sims 2");
#endif
    ...
    QString ts2DocumentsDirStr = QStandardPaths::locate(QStandardPaths::DocumentsLocation,
                                                        QString("EA Games") + QDir::separator() + ts2DisplayName,
...
void Sims2Settings::resizeEvent(QResizeEvent* event)
{
    resizeRefreshRateWarning();
    AbstractSettingsWidget::resizeEvent(event);
}

void Sims2Settings::showEvent(QShowEvent* event)
{
    resizeRefreshRateWarning();
    AbstractSettingsWidget::showEvent(event);
}

void Sims2Settings::resizeRefreshRateWarning()
{
