...
QDir SimsLSGameWriter::findGameDirectory() const
{
#ifdef Q_OS_WIN32
    // This really only works on Windows...
    // Order based on "Suppression Exe" contents
    QStringList exes = QStringList() << "SimsLS.exe"
                                     << "SimsLSPace.exe";
...
QFileInfo SimsLSGameWriter::gameExecutable(const QDir& gameDirectory) const
{
...
QFileInfo SimsLSGameWriter::rulesFileName(const QDir& gameDirectory) const
{
    return findFile(gameDirectory, QStringList()
...
QFileInfo SimsLSGameWriter::databaseFileName(const QDir& gameDirectory) const
{
    return findFile(gameDirectory, QStringList()
...
QFileInfo SimsLSGameWriter::findFile(QDir baseDir, QStringList options) const
{
...
setb forceMediumDefaultLow      false
...
if (not $useSoftwareRasterizer)
 ...
 if (match("${cardVendor}", "NVIDIA"))
    ...
    if (match("${cardName}", "*GeForce?Go*"))
       setb forceMediumDefaultLow true
       setb forceLowSettings   true
    endif
    ...
    if ($maxVertexProgramVersionHWMajor = 2)
       ...
       if (match("${cardName}", "GeForceFX*") or match("${cardName}", "*GeForce?FX*"))
          ...
          if (match("${cardName}", "*5700LE*"))
             log $logGroup $logLevelInfo "Forcing Medium Object detail on 5700LE"
             setb forceLowSettings    true
          endif
 ...
 elseif (match("${cardVendor}", "ATI"))
    ...
    boolProp presentWorkaround  true
      ...
      if (match("${cardName}", "*Radeon?VE*") or match("${cardName}", "*Radeon?7?00*") or match("${cardName}", "*R100*") or match("${cardName}", "*IGP 3?0*") or match("${cardName}", "*9100 IGP*") or match("${cardName}", "*XPRESS 200*")))EOF";
      }
      else {
          // Original value
          stream << R"EOF(
      if (match("${cardName}", "*Radeon?VE*") or match("${cardName}", "*7?00*") or match("${cardName}", "*R100*") or match("${cardName}", "*IGP 3?0*") or match("${cardName}", "*9100 IGP*") or match("${cardName}", "*XPRESS 200*")))EOF";
      }
      ...
      stream << R"EOF(
        ...
        if (match("${cardName}", "*9100 IGP*") or match("${cardName}", "*XPRESS 200*"))
...
if (not $useSoftwareRasterizer)
   ...
   # screen res defaults
   if (($maxVertexProgramVersionHWMajor >= 1) and ($textureMemory >= 128) and not $forceLowResolution and not $forceMediumDefaultLow)
      setOption ScreenModeResolution $High
   elseif ($textureMemory >= 64 and not $forceLowResolution)
      if (($memory <= $memoryLevelLow) and ($cpuSpeed <= $cpuLevelLow) or $forceMediumDefaultLow)
...
SimsLSGameWriter::~SimsLSGameWriter()
{
}

#include "moc_simslsgamewriter.cpp"
