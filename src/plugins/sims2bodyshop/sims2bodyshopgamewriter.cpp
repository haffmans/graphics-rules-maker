...
#include "sims2bodyshopgamewriter.h"
...
QDir Sims2BodyShopGameWriter::findGameDirectory() const
{
#ifdef Q_OS_WIN32
    // This really only works on Windows...
    // Order based on "Suppression Exe" contents
    // This actually finds the installation path of the Sims 2, to locate the proper installation of Body Shop.
    // TODO Check if this works for disc-based (non-Ultimate Collection) versions of the game
    ...
    qInfo() << "Trying to find The Sims 2 (for BodyShop) through the registry...";
    QString result;
...
QFileInfo Sims2BodyShopGameWriter::gameExecutable(const QDir& gameDirectory) const
{
    return findFile(gameDirectory, QStringList()
        // Sims 2 Ultimate Collection with store
        << "Fun with Pets/EP99/CSBin/TS2BodyShop.exe"
        // Sims 2 Ultimate Collection without store
        << "Fun with Pets/SP9/CSBin/TS2BodyShop.exe"
        // Fun with Pets/Best of Business/University Life discs has store edition
        << "EP99/CSBin/TS2BodyShop.exe"
        // Fun with Pets download - EP9 exe
        << "SP9/CSBin/TS2BodyShop.exe"
        // Best of Business download - SP7 exe(?)
        << "SP7/CSBin/TS2BodyShop.exe"
        // University Life download - SP8 exe(?)
        << "SP8/CSBin/TS2BodyShop.exe"
        // Directory to game/EP/SP installation directly
        << "CSBin/TS2BodyShop.exe"
    );
}

QFileInfo Sims2BodyShopGameWriter::rulesFileName(const QDir& gameDirectory) const
{
    return findFile(gameDirectory, QStringList()
        // Sims 2 Ultimate Collection with store
        << "Fun with Pets/EP99/TSData/Res/CSConfig/Graphics Rules.sgr"
        // Sims 2 Ultimate Collection without store
        << "Fun with Pets/SP9/TSData/Res/CSConfig/Graphics Rules.sgr"
        // Fun with Pets/Best of Business/University Life discs has store edition
        << "EP99/TSData/Res/CSConfig/Graphics Rules.sgr"
        // Fun with Pets download - EP9 exe
        << "SP9/TSData/Res/CSConfig/Graphics Rules.sgr"
        // Best of Business download - SP7 exe(?)
        << "SP7/TSData/Res/CSConfig/Graphics Rules.sgr"
        // University Life download - SP8 exe(?)
        << "SP8/TSData/Res/CSConfig/Graphics Rules.sgr"
        // Directory to game/EP/SP installation directly
        << "TSData/Res/CSConfig/Graphics Rules.sgr"
    );
}

QFileInfo Sims2BodyShopGameWriter::databaseFileName(const QDir& gameDirectory) const
{
    return findFile(gameDirectory, QStringList()
        // Sims 2 Ultimate Collection with store
        << "Fun with Pets/EP99/TSData/Res/CSConfig/Video Cards.sgr"
        // Sims 2 Ultimate Collection without store
        << "Fun with Pets/SP9/TSData/Res/CSConfig/Video Cards.sgr"
        // Fun with Pets/Best of Business/University Life discs has store edition
        << "EP99/TSData/Res/CSConfig/Video Cards.sgr"
        // Fun with Pets download - EP9 exe
        << "SP9/TSData/Res/CSConfig/Video Cards.sgr"
        // Best of Business download - SP7 exe(?)
        << "SP7/TSData/Res/CSConfig/Video Cards.sgr"
        // University Life download - SP8 exe(?)
        << "SP8/TSData/Res/CSConfig/Video Cards.sgr"
        // Directory to game/EP/SP installation directly
        << "TSData/Res/CSConfig/Video Cards.sgr"
    );
}
...
QFileInfo Sims2BodyShopGameWriter::findFile(QDir baseDir, QStringList options) const
{
    foreach(const QString &option, options) {
        ...
        if (QFileInfo::exists(path)) {
            ...
            qDebug() << "Sims 2 Body Shop: File" << path << " NOT found";
        }
    }
...
if (not $useSoftwareRasterizer)
   ...
   elseif (match("${cardVendor}", "ATI"))
      ...
      if (match("${cardName}", "*Radeon?VE*") or match("${cardName}", "*Radeon?7?00*") or match("${cardName}", "*R100*") or match("${cardName}", "*IGP 3?0*")))EOF";
      }
      else {
          // Original value
          stream << R"EOF(
      if (match("${cardName}", "*Radeon?VE*") or match("${cardName}", "*7?00*") or match("${cardName}", "*R100*") or match("${cardName}", "*IGP 3?0*")))EOF";
      }
...
option SimulatorControls
   ...
   setting $Medium
      intProp maxNumOfVisitingSims   4
...
if (not $useSoftwareRasterizer)
   ...
   if (($memory >= $memoryLevelHigh) and ($cpuSpeed >= $cpuLevelHigh) and ($maxVertexProgramVersionHWMajor >= 1) and not $forceLowSettings)
      ...
      setOption LotSkirtSettings      $High
   ...
   elseif (($memory >= $memoryLevelMedium) and ($cpuSpeed >= $cpuLevelMedium) and ($maxVertexProgramVersionHWMajor >= 1) and not $forceLowSettings)
      ...
      setOption LotSkirtSettings      $Medium
      ...
      setOption LotSkirtSettings      $Low
...
Sims2BodyShopGameWriter::~Sims2BodyShopGameWriter()
{
}

#include "moc_sims2bodyshopgamewriter.cpp"
