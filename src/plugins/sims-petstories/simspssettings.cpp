...
#ifdef Q_OS_WIN32
#ifdef _WIN64
    QSettings ts2Settings("HKEY_LOCAL_MACHINE\\Software\\WOW6432Node\\Electronic Arts\\The Sims 2", QSettings::NativeFormat);
#else
    QSettings ts2Settings("HKEY_LOCAL_MACHINE\\Software\\Electronic Arts\\The Sims 2", QSettings::NativeFormat);
#endif
    const auto ts2DisplayName = ts2Settings.value("Display Name").toString();
#else
    const auto ts2DisplayName = QString("The Sims™ Pet Stories");
#endif
