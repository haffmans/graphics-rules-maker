...
QDir SimsCSGameWriter::findGameDirectory() const
{
#ifdef Q_OS_WIN32
    // This really only works on Windows...
    // Order based on "Suppression Exe" contents
    QStringList exes = QStringList() << "SimsCS.exe";
...
QFileInfo SimsCSGameWriter::gameExecutable(const QDir& gameDirectory) const
{
...
QFileInfo SimsCSGameWriter::rulesFileName(const QDir& gameDirectory) const
{
    return findFile(gameDirectory, QStringList()
...
QFileInfo SimsCSGameWriter::databaseFileName(const QDir& gameDirectory) const
{
    return findFile(gameDirectory, QStringList()
...
QFileInfo SimsCSGameWriter::findFile(QDir baseDir, QStringList options) const
{
...
SimsCSGameWriter::~SimsCSGameWriter()
{
}

#include "moc_simscsgamewriter.cpp"
